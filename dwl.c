...
int status_update(void *data)
{
	char status[sizeof(stext)];
	slstatus_update(status, sizeof(status));

	strncpy(stext, status, sizeof(stext));
	drawbars();
	if (showbar_monitor_num > 0) {
		wl_event_source_timer_update(status_event_source, status_update_interval);
	}
